...
(
	SELECT
		...
		,CASE -- Determinando a tolerância. Dependendo da base do dia e dos backups anteriores, a tolerância informada pelo usuário pode ser ignorada.
			...
			WHEN TL.BackupType = 'I' AND (LB.TimeNoFull < TL.TimeTolerance OR DATEPART(weekday,CURRENT_TIMESTAMP) = 2) THEN TL.InfiniteTime -- Se o backup FULL tiver sido dentro da tolerância do DIFF, então desconsidera OU SE HOJE É SEGUNDA (TEMPORARIAMENTE)
