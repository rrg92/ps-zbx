...
{
	[CmdLetBinding()]
	param($serverInstance = $null,$database = "master",$logon=@{AuthType="Windows";User="";Password=""},$appName="CUSTOM_MSSQL_POWERSHELL", $exString = "")
	...
	$execute =  [scriptblock]::create({
					...
					try {
						...
						$ConnectionEventIdentification = [Guid]::NewGuid();
						$EvID = "SQL_$ConnectionEventIdentification"
						
						$InfoMessageScript = {
								param($Identification,$Output=$null)
								
								while($ev  = Wait-Event -SourceIdentifier $Identification){
									$InfoMessage = $ev.SourceEventArgs
									write-verbose ("	INFO MESSAGE: "+$InfoMessage.Message) 
								}
						}
						
						try{
							write-verbose "Executing the command on the connection..."
							$AsyncTask = $commandTSQL.ExecuteReaderAsync()

							Register-ObjectEvent -InputObject $this._session -Event InfoMessage -SourceIdentifier $EvID 
							
							do {
								$ev = $null
								write-verbose "Waiting Events..."
								. $InfoMessageScript -Identification $EvID
							} while(!$AsyncTask.IsCompleted)
							
							. $InfoMessageScript -Identification $EvID

						} catch {
							if(Get-EventSubscriber -SourceIdentifier $ConnectionEventIdentification){
								write-verbose "Unregistering event $EvID"
								Unregister-Event -SourceIdentifier $EvID 
							}
